...
On the Mac, run ```colima stop```. When complete run ```colima start --cpu 4 --memory 8```.
...
From the **lab06** root folder:

```bash
docker run -d --name cordwell-mlflow -p 5001:5000 -v "$PWD/mlflow_server_data:/mlflow" ghcr.io/mlflow/mlflow:v3.15.1 mlflow server --host 0.0.0.0 --port 5000 --backend-store-uri sqlite:////mlflow/mlflow.db --artifacts-destination /mlflow/artifacts
```
